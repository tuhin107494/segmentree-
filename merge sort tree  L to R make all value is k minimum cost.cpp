...
///====================================StartHere=================////////////
vector<ll> tr[2][4*mx5];
ll a[mx5];
vector<ll>ans;
vector<ll>pre;
ll cs=0;
void  merg(vector<ll> v,vector<ll>v2)
{
...
void build(int u,int b,int e)
{
    ...
    merg(tr[0][(2*u)],tr[0][(2*u)+1]);
    tr[0][u]=ans;
    tr[1][u]=pre;
 
    
}
ll quary(int u,int b,int e,int i,int j,ll k)
{
    if(e<i or b>j)return 0;
    else if(b>=i and e<=j)
    {
        ll lenth=upper_bound(all(tr[0][u]),k)-lower_bound(all(tr[0][u]),-mx18);

       ll d=lenth;
       ll ps=0;
   //   cout<<lenth<<endl;
//
       //cout<<tr[1][u].size()<<endl;
      // cout<<tr[1][u][lenth-1]<<endl;
       if(lenth>0) ps=tr[1][u][lenth-1];
       d*=k;
...
ll quary2(int u,int b,int e,int i,int j,ll k)
{
    if(e<i or b>j)return 0;
    else if(b>=i and e<=j)
    {
        ll lenth=upper_bound(all(tr[0][u]),1e18)-lower_bound(all(tr[0][u]),k+1);
         ll d=lenth;
         ll sz=e-b+1;
         //cout<<sz<<" "<<d<<endl;
    ll ps=0;
       if(sz>0)ps=tr[1][u][sz-1];

       if(sz-d-1>=0)ps-=tr[1][u][sz-d-1];
     // cout<<ps<<endl;
       d*=k;
    ...
    int   mid=(b+e)/2;
    ll  left=quary2(2*u,b,mid,i,j,k);
    ll right=quary2((2*u)+1,mid+1,e,i,j,k);
    return  (left+right);
}
void input()
{
    ...
    while(m--)
    {
        ll l,r,k;
        cin>>l>>r>>k;
        ans=quary(1,0,n-1,l-1,r-1,k)+quary2(1,0,n-1,l-1,r-1,k);
        cout<<ans<<endl;
    }
