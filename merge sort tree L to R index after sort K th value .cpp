...
ll quary(int u,int b,int e,ll i,ll  j,ll k)
{
    

    if(b==e)return tr[u][0];

    int   mid=(b+e)/2;

    int sz=upper_bound(all(tr[2*u]),j)-lower_bound(all(tr[2*u]),i);

    if(sz>=k)return quary(2*u,b,mid,i,j,k);
    else return quary((2*u)+1,mid+1,e,i,j,k-sz);
...
void input()
{
 ...
 cin>>n>>m;
   ...
   while(m--)
   {
    ...
    ll lf,rf,k;
    cin>>lf>>rf>>k;
    cout<<a[quary(1,0,n-1,lf-1,rf-1,k)]<<endl;
   }
