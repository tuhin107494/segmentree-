...
ll quary(int u,int b,int e,int i,int j,ll carry=0)
{
    if(e<i or b>j)return 0;
    else if(b>=i and e<=j)
        {
            return tr[u].sum+(carry*1LL*(e-b+1LL));
        }
        int   mid=(b+e)/2;
        ll  left=quary(2*u,b,mid,i,j,carry+tr[u].propo);
        ll right=quary((2*u)+1,mid+1,e,i,j,carry+tr[u].propo);
        return  (left+right);
}
void  update (int u,int b,int e,int i,int j,ll x)
{
    if(e<i or b>j)return;
        ...
        tr[u].sum=(tr[2*u].sum+tr[2*u +1].sum)+(1LL*(e-b+1)*tr[u].propo);
}
